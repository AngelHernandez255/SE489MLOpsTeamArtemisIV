...
Artemis Movie Recomender is a machine learning project that implements A movie recomender that looks at review data and gives recomendations for movies to watch..
...
This template uses the modern **`src/` layout** — the importable package lives in `src/SE489MLOpsTeamArtemisIV/`, decoupled from the repository root. That forces `pip install -e .` before imports work, which catches packaging bugs early.

```
SE489MLOpsTeamArtemisIV/                  # Repository root
├── src/
│   └── SE489MLOpsTeamArtemisIV/          # Importable Python package
...
Data and model artifacts are accessed via the constants in `SE489MLOpsTeamArtemisIV.config` (`PROJECT_ROOT`, `DATA_DIR`, `MODELS_DIR`, …) rather than relative paths — code is independent of where you invoke it from.
