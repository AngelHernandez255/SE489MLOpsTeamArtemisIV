
# Movie Recommender
## Team ArtemisIV Members (Name & Email):
### Angel Hernandez (AHERN255@depaul.edu)
### Anjan Kumar Basavaraj Gurudatt (ABASAVAR@depaul.edu)
### Sakshi Gorkhali (SGORKHAL@depaul.edu)
### Joshua Nevin Chandrasekar (JCHANDR2@depaul.edu)
## SE 489: ML Engineering For Production (MLOps)

# Overview
This project is an ML-powered movie-recommending software. 
It uses a dataset of movie reviews to recommend movies to a user.
"Problem statement and motivation"
"Main objectives"

# Project Architecture Diagram

in progress...

# Phase Deliverables

in progress...

# Setup Instructions

 How to set up the environment (conda/pip, requirements.txt, Docker, etc.)
 How to run the code and reproduce results

# Contribution Summary

Anjan Kumar Basavaraj Gurudatt - 
Sakshi Gorkhali	- 
Joshua Nevin Chandrasekar - 
Angel Henrnadez - GitHub Repo Manager and front-end development

# References
