# Getting Started with Artemis Movie Recomender
...
**Using conda:**
```bash
conda create -n SE489MLOpsTeamArtemisIV python=3.13
conda activate SE489MLOpsTeamArtemisIV
```
...
```bash
python -m SE489MLOpsTeamArtemisIV.data.make_dataset
```
...
```bash
python -m SE489MLOpsTeamArtemisIV.predict_model --model-path models/model.pkl --input data/test.csv
```
...
pytest tests/ --cov=SE489MLOpsTeamArtemisIV
...
```bash
docker build -t SE489MLOpsTeamArtemisIV -f dockerfiles/Dockerfile .
```
...
├── src/
│   └── SE489MLOpsTeamArtemisIV/          # Importable package (src/ layout)
...
- Contact TeamAtremisIV by Anjan, Sakshi, Joshua, Angel. (AHERN255@depaul.edu ABASAVAR@depaul.edu SGORKHAL@depaul.edu JCHANDR2@depaul.edu)
